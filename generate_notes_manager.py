...
import { PersonalNote, NoteHistory, Person, Invoice } from "../../types";
...
export function PersonalNotesManager({ storeSettings }: any) {
  ...
  const filteredPersons = systemPersons.filter(p => p.name.includes(personSearch) || p.code.includes(personSearch)).slice(0, 5);
