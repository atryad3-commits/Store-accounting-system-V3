      ...
      <table className="w-full">
        <thead>
          <tr>
            <td>
              ...
              {/* Customer Info */}
              <div className={\`grid grid-cols-2 \${isA5 ? 'gap-2 mb-4' : 'gap-4 mb-8'}\`}>
        ...
        <tbody>
          <tr>
            <td>
              {/* Items Table */}
              <div className={\`\${isA5 ? 'mb-4' : 'mb-8'} rounded-lg overflow-hidden \${isClassic ? 'border-2 border-slate-800' : 'border border-slate-200'}\`}>
                <table className="w-full text-right text-sm">
                  <thead className={\`\${isClassic ? 'border-b-2 border-slate-800 bg-slate-50' : 'border-b border-slate-200 bg-slate-50'} \${isA5 ? 'text-[10px]' : 'text-sm'}\`}>
                    <tr>
                      <th className={\`\${isA5 ? 'py-1 px-2' : 'py-3 px-4'} font-black text-center w-12\`}>ردیف</th>
                  ...
                  <tbody className={\`divide-y \${isClassic ? 'divide-slate-800' : 'divide-slate-200'}\`}>
                    {data.items?.map((item: any, idx: number) => (
                      <tr key={idx} className="print-avoid-break">
                        <td className={\`\${isA5 ? 'py-1 px-2 text-[10px]' : 'py-3 px-4'} text-center font-bold\`}>{toPersianDigits(idx + 1)}</td>
                        ...
                        <td className={\`\${isA5 ? 'py-1 px-2 text-xs' : 'py-3 px-4 text-base'} text-center font-black\`}>{toPersianDigits(addCommas(item.totalPrice || 0))}</td>
                      </tr>
                    ))}
...
fs.writeFileSync('src/components/print/invoice-templates/StandardInvoiceTemplate.tsx', content);
console.log("Rewrote StandardInvoiceTemplate.tsx");
