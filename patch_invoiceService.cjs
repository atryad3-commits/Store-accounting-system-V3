...
code = code.replace(
/description: \`دریافت وجه به مبلغ \$\{formattedAmount\} واریز به \$\{resourceName\} بابت رسید دریافت شماره \$\{newTransaction\.receiptNumber \|\| newTransaction\.id\}\`/g,
...
code = code.replace(
/description: \`طرف حساب \$\{personName\} به مبلغ \$\{formattedAmount\} بابت رسید دریافت شماره \$\{newTransaction\.receiptNumber \|\| newTransaction\.id\}\`/g,
...
code = code.replace(
/description: \`طرف حساب \$\{personName\} به مبلغ \$\{formattedAmount\} بابت رسید پرداخت شماره \$\{newTransaction\.receiptNumber \|\| newTransaction\.id\}\`/g,
...
code = code.replace(
/description: \`پرداخت وجه به مبلغ \$\{formattedAmount\} از \$\{resourceName\} بابت رسید شماره \$\{newTransaction\.receiptNumber \|\| newTransaction\.id\}\`/g,
