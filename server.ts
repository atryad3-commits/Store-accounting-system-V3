...
async function startServer() {
  ...
  app.get('/api/data/cashflow-forecast', async (req, res) => {
    try {
      ...
      const pendingIssued = await db.select().from(issuedChecks).where(
        and(
      ...
      const pendingReceived = await db.select().from(receivedChecks).where(
        and(
          inArray(receivedChecks.status, ['received', 'deposited']),
