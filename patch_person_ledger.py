...
# Locate the return statement for the table.
# It starts with:
table_start_str = """                                  return (
                                            ...
                                            <div key={index} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col gap-2 relative">
                                              ...
                                              <div className="grid grid-cols-2 gap-2 text-xs mb-2">
                                                <div className="flex flex-col gap-1 p-2 bg-rose-50 rounded-xl">
                                                  ...
                                                  <span className="font-black text-rose-700">{entry.debit > 0 ? toPersianDigits(formatCurrency(entry.debit)) : "-"}</span>
                                                </div>
                                                <div className="flex flex-col gap-1 p-2 bg-emerald-50 rounded-xl">
                                                  ...
                                                  <span className="font-black text-emerald-700">{entry.credit > 0 ? toPersianDigits(formatCurrency(entry.credit)) : "-"}</span>
                                                </div>
                                              </div>
                                              ...
                                              <div className="flex justify-between items-center bg-slate-50 p-2 rounded-xl mt-1">
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                  <span className="font-black text-slate-800">
                                                    {toPersianDigits(formatCurrency(Math.abs(entry.balance)))}
                                                  </span>
...
content = content.replace(table_start_str, table_replacement)

# We also need to find where the table ends and close the extra <div>s.
# The table is closed with </table>
# Let's see how many </table> there are. 
