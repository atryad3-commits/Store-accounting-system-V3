...
export default function SaleReturnInvoiceCreate(props: any) {
  const {
            ...
            {editingInvoiceId && (
              <div className="bg-amber-50 text-amber-900 px-5 py-4 rounded-2xl flex items-center justify-between gap-3 border border-amber-200/60 font-bold shadow-xs">
                <div className="flex items-center gap-2.5">
                  ...
                  <span>
                    شما در حال ویرایش فاکتور برگشت از فروش پیش‌نویس/ثبت‌شده
                    شماره{" "}
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border-2 border-indigo-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {storeSettings.requireWarehouse && (
                  <div className="lg:col-span-1">
                    <label className="block text-sm font-bold text-slate-600 mb-2 flex items-center gap-1.5">
                      <Box className="w-4 h-4 text-indigo-500" /> انبار ورود
                      کالا
                    </label>
                    <select
                      value={invoiceWarehouseId}
                      onChange={(e) => setInvoiceWarehouseId(e.target.value)}
                      className="w-full p-3 border border-indigo-100 rounded-xl focus:ring-2 focus:ring-indigo-500 bg-indigo-50/30 text-base font-bold text-indigo-900 outline-none"
                    >
